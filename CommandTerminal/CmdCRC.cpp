...
CmdCRC::CmdCRC()
:
  Command("CRC Checking", "AT+CRC",
#if defined(TARGET_MTS_MDOT_F411RE)
    "Enable/disable CRC (0: off, 1: on)",
#else
    "",
#endif
  "(0,1)")
...
uint32_t CmdCRC::action(const std::vector<std::string>& args)
                        {
    if (args.size() == 1) {
...
bool CmdCRC::verify(const std::vector<std::string>& args)
                    {
    if (args.size() == 1)
        return true;
